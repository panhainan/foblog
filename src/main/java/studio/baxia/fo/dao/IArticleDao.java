...
public interface IArticleDao {
    ...
    /**
     * 插入文章信息（待编写sql实现）
     * @param article 文章信息（title,summary,content,categoryIds,tagIds,status,writeTime,pubTime）
     * @return 受影响的行数
     */
    ...
    /**
     * 更新文章
     * @param article 文章信息（id,[title,summary,content,categoryIds,tagIds,status,writeTime,pubTime](可为空，为空表示该字段不修改)）
     * @return 受影响的行数
     */
     ...
     * @param article 文章信息（此处可选包括：title、categoryIds、tagIds、status）
    ...
    /**
     * 通过标题、类别id、标签id进行模糊查找取得所有记录数量。
     * 注意：参照"selectBy"方法的注意事项，用于配合"selectBy"方法使用
     * @param article 文章信息（此处可选包括：title、categoryIds、tagIds）
     * @return
     */
     ...
     * @return
     */
    ArticleVo selectVoByTitle(@Param("title")String articleTitle,@Param("status")Integer articleStatus);


    Article selectNextOrPreVoBy(@Param("article")Article article, @Param("nextOrPreVo")Boolean nextOrPreVo);

	Integer updateCategoryId(@Param("oldCategoryId")Integer oldCategoryId,@Param("newCategoryId")Integer newCategoryId);
	/**
	 * 获取所有归档，即获取每年或者每月的归档
	 * @param articleStatus 文章状态，传值设定为blog状态 . CommonConstant类中有对应的常量
	 * @param archiveType 归档类型：%Y年   %Y年%m月 . CommonConstant类中有对应的常量
	 * @return
	 */
	List<ArchiveVo> selectAllArchives(@Param("status")Integer articleStatus,@Param("archive_type")String archiveType);
	
	/**
	 * 获取某归档对应的所有文章
	 * @param articleStatus 文章状态，传值设定为blog状态 . CommonConstant类中有对应的常量
	 * @param archiveType 归档类型：%Y年   %Y年%m月 . CommonConstant类中有对应的常量
	 * @param monthName 归档中的具体信息，如：2016年10月
	 * @return
	 */
	List<Article> selectArchiveArticles(@Param("status")Integer articleStatus,@Param("archive_type")String archiveType,@Param("month_name")String monthName);
}
